...
def render_prediction_tab():
    ...
    with col2:
        ...
        tenure = st.number_input("Meses de antigüedad (Tenure)", min_value=0, max_value=100, value=1)
    ...
    with fin_col1:
        monthly_charges = st.number_input("Cargos Mensuales ($)", min_value=0.0, value=29.85)
    with fin_col2:
        total_charges = st.number_input("Cargos Totales ($)", min_value=0.0, value=29.85)
    ...
    if st.button("🚀 Evaluar Riesgo de Fuga", type="primary", use_container_width=True):
        ...
        payload = {
            ...
            "DeviceProtection": "No" if is_no_internet else "Yes", # Simplificado por UX
        ...
        with st.spinner("Consultando al motor de inferencia..."):
            try:
                ...
                st.error("❌ No se pudo conectar a la API. Asegúrese de que FastAPI esté corriendo en el puerto 8000.")
...
def display_prediction_result(result: dict):
    ...
    with col2:
        ...
        st.markdown(f"<h3 style='color:{color}; text-align:center;'>Riesgo de Fuga: {risk_score:.1%}</h3>", unsafe_allow_html=True)
    ...
    if will_churn:
        st.warning("Se recomienda enviar una oferta de retención o contactar al cliente inmediatamente.")


def render_analytics_tab():
    """Muestra las métricas y explicabilidad del modelo pre-generadas."""
    st.header("Auditoría del Modelo (Explainable AI)")
    st.write("Este panel muestra el rendimiento del modelo Campeón activo (CatBoost) y explica cómo toma sus decisiones.")

    img_dir = "docs/assets/images"
    
    if not os.path.exists(img_dir):
        st.error("No se encontraron las imágenes. Por favor ejecute la evaluación del modelo primero.")
        return
